#' GSEAweight2
#'
#' The implementation of GSEAweight2
#'
#' @details `GSEAweight2Score()` returns a data.frame, each row of which contains score,
...
#######################The implementation of GSEAweight2########################
GSEAweight2Score <- function(refMatrix, queryUp, queryDown, permuteNum = 10000,
  ...
  weight2EnrichmentScore <- function(refList, query) {
  ...
  weight2 <- function(refList, queryUp, queryDown) {
    scoreUp <- weight2EnrichmentScore(refList, queryUp)
    scoreDown <- weight2EnrichmentScore(refList, queryDown)
  ...
  score <- mclapply(refList, weight2, queryUp = queryUp, queryDown = queryDown,
                    mc.cores = mcCore)
  ...
  for(n in 1:permuteNum) {
    ...
    bootScore <- mclapply(refList, weight2, queryUp = bootUp,
